...
@Composable
fun MovementDetailContent(reserva: ReservaDetail) {
    Column(
        modifier = Modifier
            .fillMaxSize()
        ...
        Card(
            ...
            Column(modifier = Modifier.padding(24.dp)) {
                ...
                Text(
                    text = formatDetailCurrency(reserva.MontoTotal ?: 0.0),
                    fontSize = 32.sp,
                    fontWeight = FontWeight.Bold,
                    color = Color(0xFF2C3E50)
                )
        ...
        InfoCard {
            InfoRow(label = "Fecha y Hora Entrada", value = formatDetailDate(reserva.fechaIngreso ?: 0))
            Divider(color = Color(0xFFF0F0F0))
            InfoRow(label = "Fecha y Hora Salida", value = if (reserva.fechaSalida != null && reserva.fechaSalida > 0) formatDetailDate(reserva.fechaSalida) else "--")
            Divider(color = Color(0xFFF0F0F0))
            InfoRow(label = "Personal", value = reserva.nombreResponsable ?: "Desconocido")
        }
        ...
        InfoCard {
            val vehicleInfo = "${reserva.tipoVehiculo ?: ""} ${reserva.categoriaVehiculo ?: ""}".trim()
            InfoRow(label = "Vehículo", value = if (vehicleInfo.isBlank()) "Desconocido" else vehicleInfo)
            Divider(color = Color(0xFFF0F0F0))
            InfoRow(label = "Placas", value = reserva.placaVehiculo ?: "N/A")
        }
...
fun BottomActionButtons() {
    Surface(
        ...
        Row(
            ...
            // Cobrar Button
            Button(
                onClick = { /* Handle Cobrar */ },
                colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF66D19E)), // Greenish
                shape = RoundedCornerShape(4.dp),
                modifier = Modifier
                    .weight(1f)
                    .height(56.dp)
